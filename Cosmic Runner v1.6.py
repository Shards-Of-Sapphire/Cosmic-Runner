        ...
        if self.type == "ground":
            if self.biome == PLATEAU:
                ...
                for _ in range(8):
                    ...
                    pygame.draw.circle(self.image, (248, 213, 183), (x, y), 1)
                ...
                for _ in range(8):
                    x = random.randint(0, TILE_SIZE-1)
                    y = random.randint(0, TILE_SIZE-1)
                    pygame.draw.circle(self.image, (135, 206, 250), (x, y), random.randint(1, 2))
            
            elif self.biome == SNOW:
                # Snow ground with depth
                self.image.fill(SNOW_WHITE)
                # Add snow texture and shadows
                for _ in range(6):
                    x = random.randint(0, TILE_SIZE-1)
                    y = random.randint(0, TILE_SIZE-1)
                    size = random.randint(1, 3)
                    pygame.draw.circle(self.image, (240, 240, 250), (x, y), size)
            
            elif self.biome == VOLCANO:
                self.image.fill((70, 35, 35))
                for _ in range(5):
                    x = random.randint(2, TILE_SIZE-3)
                    y = random.randint(2, TILE_SIZE-3)
                    pygame.draw.circle(self.image, (255, 50, 0), (x, y), 1)
            
            elif self.biome == SKY:
                self.image.fill((200, 200, 255, 180))
                for _ in range(5):
                    x = random.randint(0, TILE_SIZE-1)
                    y = random.randint(0, TILE_SIZE-1)
                    pygame.draw.circle(self.image, (255, 255, 255), (x, y), random.randint(4, 8))
                ...
                for _ in range(4):
                    x = random.randint(0, TILE_SIZE-1)
                    y = random.randint(0, TILE_SIZE-1)
                    pygame.draw.circle(self.image, (80, 80, 120), (x, y), random.randint(2, 6))
                # Add some alien crystal formations
                for _ in range(2):
                    x = random.randint(0, TILE_SIZE-1)
                    y = random.randint(0, TILE_SIZE-1)
                    pygame.draw.circle(self.image, (150, 100, 255), (x, y), 1)
...
def draw_instructions(screen):
    ...
    # Title with glow
    for i in range(3):
        title_glow = font_huge.render("GAME INSTRUCTIONS", True, (50 + i*30, 50 + i*20, 100 + i*50))
        title_glow_rect = title_glow.get_rect(center=(SCREEN_WIDTH//2 + i, 70 + i))
        screen.blit(title_glow, title_glow_rect)
    
    title_text = font_huge.render("GAME INSTRUCTIONS", True, YELLOW)
    ...
    instructions = [
        ("CONTROLS:", WHITE, True),
        ("SPACE - Jump / Use Jetpack (hold in air)", (150, 200, 255), False),
        ("P - Pause Game", (150, 200, 255), False),
        ("M - Toggle Music On/Off", (150, 200, 255), False),
        ("F11 - Toggle Fullscreen Mode", (150, 200, 255), False),
        ("", WHITE, False),
        ("GAMEPLAY:", WHITE, True),
        ("• Navigate through 7 unique cosmic biomes", (200, 200, 200), False),
        ("• Collect coins to increase your score", (200, 200, 200), False),
        ("• Avoid obstacles or lose a life", (200, 200, 200), False),
        ("• Complete biome-specific missions for rewards", (200, 200, 200), False),
        ("• Find checkpoints to save progress", (200, 200, 200), False),
        ("• Smart obstacle spacing ensures fair gameplay", (200, 200, 200), False),
        ("", WHITE, False),
        ("BIOME PROGRESSION:", WHITE, True),
        ("Plateau → Dark Forest → Desert → Sea", (255, 200, 100), False),
        ("→ Volcano → Sky → 🚀 SPACE (FINAL)", (255, 100, 255), False),
        ("", WHITE, False),
        ("POWER-UPS:", WHITE, True),
        ("🛡️ Blue Shield - Absorbs one hit (5s)", (100, 150, 255), False),
        ("⚡ Yellow Speed - Temporary speed boost (6s)", (255, 215, 0), False),
        ("🧲 Purple Magnet - Attracts coins (10s)", (200, 100, 200), False),
        ("💰 Green Double - Double coin rewards (8s)", (100, 255, 100), False),
        ("🚀 Orange Jetpack - Limited flight ability (5s)", (255, 140, 0), False),
        ("", WHITE, False),
        ("FEATURES:", WHITE, True),
        ("• Enhanced obstacle spacing algorithm", (150, 255, 150), False),
        ("• Animated runner with multiple frames", (150, 255, 150), False),
        ("• 7 unique biomes with special challenges", (150, 255, 150), False),
        ("• Mission system with progressive difficulty", (150, 255, 150), False),
        ("• Space biome as ultimate challenge", (255, 150, 255), False),
        ("", WHITE, False),
        ("Press ESC to return to menu", YELLOW, False)
    ]
    ...
    for text, color, is_header in instructions:
        ...
        if is_header:
            ...
            header_bg = pygame.Surface((600, 35))
            header_bg.fill((50, 50, 120))
            header_bg.set_alpha(180)
            header_rect = header_bg.get_rect(center=(SCREEN_WIDTH//2, y))
            screen.blit(header_bg, header_rect)
        
        font_to_use = font_large if is_header else font_small
        rendered_text = font_to_use.render(text, True, color)
        text_rect = rendered_text.get_rect(center=(SCREEN_WIDTH//2, y))
        screen.blit(rendered_text, text_rect)
        
        y += 40 if is_header else 28
